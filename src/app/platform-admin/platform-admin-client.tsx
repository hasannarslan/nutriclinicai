...
import { ArrowLeft, Building2, CalendarClock, CheckCircle2, Clipboard, ExternalLink, FlaskConical, Link2, MessageSquareText, RefreshCw, Send, ShieldCheck, UsersRound } from "lucide-react";
...
type Payload = { clinics: ClinicRow[]; pilotInvites: PilotInvite[]; feedback: Feedback[]; plans: Plan[]; metrics: { clinics: number; pilotClinics: number; activeUsers: number; activeClients: number; openFeedback: number } };
...
export default function PlatformAdminClient({ email }: { email: string }) {
  ...
  const [tab, setTab] = useState<"clinics" | "invites" | "feedback">("clinics");
