...
import {
  ...
  Sparkles, Target, UtensilsCrossed, WandSparkles, Weight, X, WalletCards, CreditCard, CheckCircle2,
} from "lucide-react";
...
function ClientDailyHub({ onboarding, clinicId }: { onboarding: Onboarding; clinicId: string }) {
  ...
  const load = useCallback(async () => {
    ...
    if (error) {
      ...
      const { data: paymentRows, error: paymentError } = await supabase
        .from("payments")
        .select("id,service_type,description,amount,currency,status,method,paid_at,created_at")
  ...
  const outstandingTotal = payments.filter((payment) => payment.status === "pending" || payment.status === "partial").reduce((sum, payment) => sum + Number(payment.amount), 0);
  const latestPayment = payments[0] || null;

  return <div className="v4-daily-shell">
    <div className="v4-daily-hero"><div><span className="section-kicker">GÜNLÜK TAKİP</span><h1>Bugünün planı</h1><p>{hub.plan ? `${hub.plan.title} • ${goalLabel(onboarding.primary_goal)}` : "Diyetisyeniniz aktif menü oluşturduğunda günlük hedefler burada görünür."}</p></div><div className="v4-hero-avatar"><Apple /></div></div>
    ...
    <section className={`v4-payment-overview ${outstandingTotal > 0 ? "attention" : paidTotal > 0 ? "paid" : "empty"}`}>
      ...
      <div className="v4-payment-copy">
        ...
        <p>{latestPayment ? `${latestPayment.service_type} • ${paymentMethodText(latestPayment.method)} • ${new Date(latestPayment.paid_at || latestPayment.created_at).toLocaleDateString("tr-TR")}` : "Klinik tarafından ödeme kaydı oluşturulduğunda burada görünecek."}</p>
      </div>
    ...
    <div className="v4-tracker-grid">
      ...
      <section className="v4-tracker-card activity"><div className="v4-tracker-head"><Activity /><div><span>Yakılan</span><strong>{Math.round(hub.burned_calories)} kcal</strong><small>Manuel aktivite kaydı</small></div></div><div className="v4-activity-form"><input value={activityForm.type} onChange={(e) => setActivityForm({ ...activityForm, type: e.target.value })} placeholder="Aktivite" /><input type="number" value={activityForm.duration} onChange={(e) => setActivityForm({ ...activityForm, duration: e.target.value })} placeholder="Dakika" /><input type="number" value={activityForm.calories} onChange={(e) => setActivityForm({ ...activityForm, calories: e.target.value })} placeholder="kcal" /><button onClick={addActivity}><Plus />Egzersiz ekle</button></div></section>
    </div>
  </div>;
}
...
function FoodScannerPanel({ hub, onClose }: { hub: DailyHub; onClose: () => void }) {
  ...
  return <div className="v4-tool-overlay"><section className="v4-tool-panel"><header><button onClick={onClose}><ChevronLeft /></button><div><span className="section-kicker">GIDA GÜVENLİĞİ</span><h2>Yiyecek tarayıcı</h2></div><ScanLine /></header><div className="v4-scan-upload">{preview ? <img src={preview} alt="Yüklenen ürün etiketi" /> : <Camera />}<label><ImagePlus />Fotoğraf seç<input type="file" accept="image/*" onChange={(e) => choose(e.target.files?.[0] || null)} /></label></div><p className="v4-tool-help">İçindekiler ve alerjen uyarısı net görünecek şekilde fotoğraf çekin. Sonuç kesin tıbbi güvence değildir; ambalaj etiketini ve diyetisyeninizi esas alın.</p>{message && <div className="notice-bar"><AlertTriangle size={16} />{message}</div>}{result && <div className={`v4-scan-result ${result.suitability}`}><div className="v4-scan-summary"><ShieldIcon status={result.suitability} /><div><b>{result.product_name || "Ürün analizi"}</b><p>{result.summary}</p></div></div><h3>Alerjen uyarıları</h3>{result.allergen_alerts.length ? result.allergen_alerts.map((item) => <article key={`${item.allergen}-${item.reason}`}><span className={`severity ${item.severity}`}>{item.severity}</span><div><b>{item.allergen}</b><p>{item.reason}</p></div></article>) : <p>Profilinizle eşleşen belirgin alerjen saptanmadı.</p>}<h3>Katkı maddeleri</h3>{result.additive_findings.length ? result.additive_findings.map((item) => <article key={item.code_or_name}><span>🧪</span><div><b>{item.code_or_name}</b><p>{item.note}</p></div></article>) : <p>Okunabilen etikette belirgin katkı maddesi saptanmadı.</p>}<small>{result.uncertainty_note}</small></div>}<button className="v4-primary" onClick={scan} disabled={loading}>{loading ? <LoaderCircle className="spin" /> : <ScanLine />}{result ? "Yeniden tara" : "Etiketi analiz et"}</button></section></div>;
}
