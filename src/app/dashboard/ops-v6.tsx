...
function shortDate(value:string|null|undefined){if(!value)return "—";return new Date(value).toLocaleDateString(getRuntimeIntlLocale(),{day:"2-digit",month:"short",year:"numeric"})}
...
export function DocumentsV6({role,clinicId}:{role:Role;clinicId:string}){
  ...
  async function upload(){const file=fileRef.current?.files?.[0];if(!file||!clientId)return setMessage("Dosya ve danışan seçin.");if(!form.title.trim())return setMessage("Belge başlığı girin.");const allowed=["application/pdf","application/vnd.openxmlformats-officedocument.wordprocessingml.document","image/jpeg","image/png","image/webp"];if(!allowed.includes(file.type))return setMessage("Yalnızca PDF, DOCX, JPG, PNG veya WEBP yükleyebilirsiniz.");if(file.size>15*1024*1024)return setMessage("Belge en fazla 15 MB olabilir.");setUploading(true);const {data:user}=await supabase.auth.getUser();if(!user.user){setUploading(false);return}const path=`${user.user.id}/${clientId}/${Date.now()}-${safeName(file.name)}`;const {error:storageError}=await supabase.storage.from("client-documents").upload(path,file,{contentType:file.type,upsert:false});if(storageError){setUploading(false);return setMessage(storageError.message)}const {error}=await supabase.from("client_documents").insert({clinic_id:clinicId,client_id:clientId,category:form.category,title:form.title.trim().slice(0,180),file_name:file.name.slice(0,255),storage_path:path,mime_type:file.type||null,size_bytes:file.size,document_date:form.document_date||null,notes:form.notes.trim().slice(0,2000)||null,visible_to_client:clientMode?true:form.visible_to_client,uploaded_by:user.user.id});if(error){await supabase.storage.from("client-documents").remove([path]);setMessage(error.message)}else{setForm({category:"laboratory",title:"",document_date:localDate(),notes:"",visible_to_client:true});if(fileRef.current)fileRef.current.value="";setMessage("Belge güvenli dosyaya eklendi.");await load()}setUploading(false)}
...
export function DirectMessagesV6({role,clinicId}:{role:Role;clinicId:string}){
  ...
  async function send(){const cleanBody=body.trim().slice(0,5000);const file=attachmentRef.current?.files?.[0];if(!conversationId||(!cleanBody&&!file))return;if(file){const allowed=file.type.startsWith("image/")||file.type.startsWith("audio/")||file.type==="application/pdf";if(!allowed)return setMessage("Mesaj eki yalnızca görsel, ses veya PDF olabilir.");if(file.size>10*1024*1024)return setMessage("Mesaj eki en fazla 10 MB olabilir.");}setSending(true);let path:string|null=null;const {data:user}=await supabase.auth.getUser();if(!user.user){setSending(false);return}if(file){path=`${user.user.id}/${conversationId}/${Date.now()}-${safeName(file.name)}`;const {error}=await supabase.storage.from("direct-message-media").upload(path,file,{contentType:file.type});if(error){setMessage(error.message);setSending(false);return}}const {error}=await supabase.from("direct_messages").insert({conversation_id:conversationId,sender_user_id:user.user.id,body:cleanBody||null,attachment_path:path,attachment_name:file?.name.slice(0,255)||null,attachment_type:file?.type||null});if(error){if(path)await supabase.storage.from("direct-message-media").remove([path]);setMessage(error.message)}else{setBody("");if(attachmentRef.current)attachmentRef.current.value="";await openConversation(clientId)}setSending(false)}
