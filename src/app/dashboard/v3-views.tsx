...
function money(value:number|string|null|undefined){return new Intl.NumberFormat(getRuntimeIntlLocale(),{style:"currency",currency:"TRY",maximumFractionDigits:2}).format(Number(value)||0)}
...
function shortDate(value:string){return new Date(value).toLocaleDateString(getRuntimeIntlLocale(),{day:"2-digit",month:"short",year:"numeric"})}
function dateTime(value:string){return new Date(value).toLocaleString(getRuntimeIntlLocale(),{day:"2-digit",month:"short",hour:"2-digit",minute:"2-digit"})}
...
function documentDate(value:string){return new Date(`${value}T12:00:00`).toLocaleDateString(getRuntimeIntlLocale(),{day:"2-digit",month:"long",year:"numeric"})}
...
export function OverviewV3({role,clinicId,fullName,onClaimOwner}:{role:Role;clinicId:string;fullName:string;onClaimOwner:()=>void}){
  ...
  async function remind(row:PaymentDueRow,channel:"email"|"sms"){
    setReminding(`${row.id}:${channel}`);
    try{
      ...
      if(!response.ok)throw new Error(json?.error||"Hatırlatma gönderilemedi.");
      setMessage(`${row.client_name} için ödeme hatırlatması gönderildi.`);await load();
    }catch(error){setMessage(error instanceof Error?error.message:"Hatırlatma gönderilemedi.");}
  ...
  const cards=role==="owner"?[
    ...
    {label:"Sadakat puanı",value:(summary?.loyalty_balance||0).toLocaleString(getRuntimeIntlLocale()),hint:"Kullanılabilir bakiye",Icon:Gift,tone:"violet"},
  ];
  ...
  return <>
    ...
    <div className={`overview-layout ${role==="owner"?"owner-layout":""}`}>
      ...
      {role!=="client"&&<section className="surface-card payment-due-card">
        ...
        {dueRows.length===0?<Empty text="Yaklaşan veya geciken ödeme bulunmuyor."/>:<div className="payment-due-list">{dueRows.map(row=><article key={row.id} className={row.days_remaining!=null&&row.days_remaining<0?"overdue":""}><div className="due-date-box"><b>{row.due_date?new Date(row.due_date).getDate():"—"}</b><small>{row.due_date?new Date(row.due_date).toLocaleDateString(getRuntimeIntlLocale(),{month:"short"}):"Tarih"}</small></div><div className="grow"><b>{row.client_name}</b><p>{row.service_type} • Kalan {money(row.remaining_amount??row.amount)}</p><small>{row.days_remaining==null?"Tarih belirlenmedi":row.days_remaining<0?`${Math.abs(row.days_remaining)} gün gecikti`:row.days_remaining===0?"Ödeme bugün":`${row.days_remaining} gün kaldı`}</small></div><div className="reminder-actions"><button disabled={reminding===`${row.id}:email`} onClick={()=>remind(row,"email")}><Mail size={14}/>Mail</button><button disabled={reminding===`${row.id}:sms`} onClick={()=>remind(row,"sms")}><Phone size={14}/>SMS</button></div></article>)}</div>}
      </section>}
...
export function AppointmentsV3({role,clinicId}:{role:Role;clinicId:string}){
  ...
  return <>
    ...
    <section className="surface-card appointment-scheduler-v3">
      ...
      <div className="selected-day-title"><div><span>{new Date(`${date}T12:00:00`).toLocaleDateString(getRuntimeIntlLocale(),{weekday:"long"})}</span><h3>{new Date(`${date}T12:00:00`).toLocaleDateString(getRuntimeIntlLocale(),{day:"2-digit",month:"long",year:"numeric"})}</h3></div><small>{slots.filter(x=>x.is_available).length} müsait saat</small></div>
    ...
    {showAvailability&&clinical&&<section className="availability-v3-grid">
      ...
      <div className="surface-card"><div className="surface-head"><div><span className="section-kicker">ÖZEL GÜN</span><h3>Güne özel müsaitlik</h3><p>Ek çalışma, izin veya kapalı saat tanımlayın.</p></div><CalendarDays size={20}/></div>
        ...
        <div className="schedule-list-v3">{exceptions.map(row=><article key={row.id}><div><b>{shortDate(row.starts_at)}</b><span>{new Date(row.starts_at).toLocaleTimeString(getRuntimeIntlLocale(),{hour:"2-digit",minute:"2-digit"})} — {new Date(row.ends_at).toLocaleTimeString(getRuntimeIntlLocale(),{hour:"2-digit",minute:"2-digit"})} • {row.is_available?"Ek müsait":"Kapalı"}</span>{row.reason&&<small>{row.reason}</small>}</div><button className="icon-danger" onClick={()=>deleteException(row.id)}><Trash2 size={15}/></button></article>)}</div>
      </div>
    </section>}
...
export function ClientsV3({role,clinicId}:{role:Role;clinicId:string}){
  ...
  async function addPoints(){
    ...
    if(points>=100000&&!window.confirm(`${points.toLocaleString(getRuntimeIntlLocale())} puan eklenecek. Bu yüksek tutarı onaylıyor musunuz?`))return;
  ...
  async function correctClientBalance(){
    ...
    if(!window.confirm(`${selected.full_name} sadakat bakiyesi ${next.toLocaleString(getRuntimeIntlLocale())} puan olarak düzeltilecek. Onaylıyor musunuz?`))return;
...
export function PaymentsV3({role,clinicId}:{role:Role;clinicId:string}){
  ...
  async function sendReminder(row:PaymentRow,channel:"email"|"sms"){
    ...
    try{
      ...
      const json=await response.json().catch(()=>null) as {error?:string}|null;
      if(!response.ok)throw new Error(json?.error||"Hatırlatma gönderilemedi.");
      setMessage(`${row.client_name} için ${channel==="email"?"e-posta":"SMS"} hatırlatması gönderildi.`);await load();
    }catch(error){setMessage(error instanceof Error?error.message:"Hatırlatma gönderilemedi.");}
    finally{setReminding("");}
  }
...
export function MealPlansV3({role,clinicId}:{role:Role;clinicId:string}){
  ...
  async function estimateNutrition(targetItems:MealDraftItem[]){
    const candidates=targetItems.filter(x=>x.food_name.trim());
    if(!candidates.length)return setMessage("AI hesabı için besin veya yemek adı yazın.");
    setAiCalculating(candidates.length===1?candidates[0].key:"all");setMessage("");
    try{
      const response=await fetch("/api/ai/nutrition-estimate",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({items:candidates.map(x=>({key:x.key,food_name:x.food_name,quantity_g:x.quantity_g?Number(x.quantity_g):null,portion_text:x.portion_text||null}))})});
      const json=await response.json().catch(()=>null) as {error?:string;items?:Array<Record<string,unknown>>}|null;
      if(!response.ok)throw new Error(json?.error||"AI besin hesabı yapılamadı.");
      if(!Array.isArray(json?.items))throw new Error("AI servisi geçersiz yanıt döndürdü.");
      const resultMap=new Map(json.items.map((x:any)=>[x.key,x]));
      setDraft(current=>current.map(item=>{const result=resultMap.get(item.key) as any;if(!result)return item;return{...item,quantity_g:item.quantity_g||String(result.estimated_quantity_g||""),portion_text:item.portion_text||`${result.estimated_quantity_g||""} g/ml`,calories:String(roundNutrition(result.calories)),protein:String(roundNutrition(result.protein_g)),carbs:String(roundNutrition(result.carbs_g)),fat:String(roundNutrition(result.fat_g))}}));
      setMessage(`${candidates.length} besin için Grok destekli yaklaşık değerler hesaplandı. Kaydetmeden önce klinik olarak kontrol edin.`);
    }catch(error){setMessage(error instanceof Error?error.message:"AI besin hesabı yapılamadı.");}
    finally{setAiCalculating("");}
  }
