...
function money(value:number){return new Intl.NumberFormat(getRuntimeIntlLocale(),{style:"currency",currency:"TRY"}).format(value)}
...
export function TeamV3({clinicId,currentUserId}:{clinicId:string;currentUserId:string}){
  ...
  const load=useCallback(async()=>{
    ...
    const map = new Map<string, { full_name?: string; email?: string | null; phone?: string | null }>(
      (p || []).map((x: any) => [String(x.id), x]),
    );
    setRows((m || []).map((x: any) => ({ ...x, ...(map.get(String(x.user_id)) || {}) })));
  ...
  return <>
    ...
    <section className="surface-card team-invite-v7">
      ...
      {invites.length>0&&<div className="clinic-invite-list-v7">{invites.map(invite=><article key={invite.id} className={!invite.is_active?"inactive":""}><div><b>{roleText(invite.role)}</b><code>{invite.token}</code><small>{invite.email||"E-posta kısıtı yok"} · {invite.used_count}/{invite.max_uses} kullanım · {new Date(invite.expires_at).toLocaleDateString(getRuntimeIntlLocale())}</small></div><div><button onClick={()=>copyInvite(invite.token)}>Bağlantıyı kopyala</button>{invite.is_active&&<button className="danger-action" onClick={()=>revokeInvite(invite.id)}>Pasif yap</button>}</div></article>)}</div>}
    </section>
...
export function MeasurementsV3({role,clinicId}:{role:Role;clinicId:string}){
  ...
  return <>
    ...
    {loading?<Loading/>:!latest?<Empty text="Henüz ölçüm kaydı bulunmuyor."/>:<>
      ...
      <section className="surface-card measurement-trend-v5"><div className="surface-head"><div><span className="section-kicker">GELİŞİM</span><h3>Kilo trendi</h3><p>Son {chartRows.length} ölçümün tarihsel değişimi.</p></div><span className="count-pill">{visible.length} kayıt</span></div>{chartRows.length>1?<div className="measurement-chart-shell"><svg viewBox="0 0 100 100" preserveAspectRatio="none" role="img" aria-label="Kilo değişim grafiği"><line x1="0" y1="88" x2="100" y2="88"/><polyline points={points}/>{chartRows.map((row,index)=>{const [x,y]=points.split(" ")[index].split(",");return <circle key={row.id} cx={x} cy={y} r="1.8"/>})}</svg><div className="measurement-chart-labels"><span>{new Date(chartRows[0].measured_at).toLocaleDateString(getRuntimeIntlLocale())}</span><b>{latest.weight_kg} kg</b><span>{new Date(latest.measured_at).toLocaleDateString(getRuntimeIntlLocale())}</span></div></div>:<p className="muted-line">Trend için en az iki kilo ölçümü gerekir.</p>}</section>
      <section className="surface-card"><div className="surface-head"><div><span className="section-kicker">GEÇMİŞ</span><h3>Ölçüm kayıtları</h3></div></div><div className="table-wrap modern-table-wrap"><table className="modern-table"><thead><tr><th>Tarih</th><th>Kilo</th><th>Yağ</th><th>Kas</th><th>Bel</th><th>Not</th></tr></thead><tbody>{visible.map(row=><tr key={row.id}><td><b>{new Date(row.measured_at).toLocaleDateString(getRuntimeIntlLocale())}</b><small>{new Date(row.measured_at).toLocaleTimeString(getRuntimeIntlLocale(),{hour:"2-digit",minute:"2-digit"})}</small></td><td>{row.weight_kg??"—"} kg</td><td>{row.body_fat_percent??"—"}%</td><td>{row.muscle_mass_kg??"—"} kg</td><td>{row.waist_cm??"—"} cm</td><td>{row.note||"—"}</td></tr>)}</tbody></table></div></section>
    </>}
  </>
}
...
export function LoyaltyV3({role,clinicId}:{role:Role;clinicId:string}){
  ...
  async function redeemReward(row:RewardRow){
    ...
    if(balance<row.points_cost){setMessage(`Bu ödül için ${(row.points_cost-balance).toLocaleString(getRuntimeIntlLocale())} puan daha gerekiyor.`);return;}
    if(!window.confirm(`${row.name} ödülü için ${row.points_cost.toLocaleString(getRuntimeIntlLocale())} puan kullanılsın mı?`))return;
    setBusy(true);
    ...
    setCelebration({
      ...
      detail:`Puanınız başarıyla kullanıldı. Kalan bakiyeniz ${result.balance_after.toLocaleString(getRuntimeIntlLocale())} puan. Ödülü kliniğinizde kullanabilirsiniz.`,
  ...
  async function correctBalance(){
    ...
    if(!window.confirm(`${selectedClient?.client_name||"Danışan"} sadakat bakiyesi ${next.toLocaleString(getRuntimeIntlLocale())} puan olarak düzeltilecek. Onaylıyor musunuz?`))return;
    setBusy(true);
    ...
    setMessage(`Puan bakiyesi ${result.old_balance.toLocaleString(getRuntimeIntlLocale())} puandan ${result.new_balance.toLocaleString(getRuntimeIntlLocale())} puana düzeltildi.`);
  ...
  return <>
      ...
      <div className="form-grid loyalty-issue-grid">
        <label>Danışan
          <select value={selectedClientId} onChange={event=>setSelectedClientId(event.target.value)}>
            ...
            {clients.map(client=><option key={client.client_id} value={client.client_id}>{client.client_name} · {client.member_no} · {client.remaining_points.toLocaleString(getRuntimeIntlLocale())} puan</option>)}
          </select>
        </label>
        <label>Mevcut kullanılabilir puan
          <input value={selectedClient?selectedClient.remaining_points.toLocaleString(getRuntimeIntlLocale()):"—"} readOnly/>
        </label>
      </div>
    ...
    <div className="loyalty-banner v3-loyalty-banner">
      ...
      <div>
        ...
        <strong>{role==="client"?balance.toLocaleString(getRuntimeIntlLocale()):rewards.filter(reward=>reward.is_active).length}</strong>
      </div>
    </div>
    ...
    {rewards.length===0
      ...
      {rewards.map(row=>{
        ...
        return <article key={row.id} className={role==="client"&&!eligible?"reward-locked":"reward-available"}>
          ...
          <b>{row.points_cost.toLocaleString(getRuntimeIntlLocale())} puan</b>
          <small>Stok: {row.stock===null?"Sınırsız":row.stock}</small>
          {role==="client"&&<>
            <span className={`reward-eligibility ${eligible?"eligible":"locked"}`}>
              {eligible?"Bu ödülü puanınızla hemen alabilirsiniz":row.stock===0?"Stokta yok":`${missing.toLocaleString(getRuntimeIntlLocale())} puan eksik`}
            </span>
      ...
      {redemptions.length===0?<Empty text="Henüz tanımlanmış ödül yok."/>:<div className="redemption-list">
        {redemptions.map(row=><article key={row.id}>
          ...
          <div className="grow">
            ...
            <small>{new Date(row.requested_at).toLocaleString(getRuntimeIntlLocale())} · {row.points_spent.toLocaleString(getRuntimeIntlLocale())} puan</small>
...
export function ProfileSettingsV3({profile,role,clinic,onUpdated,onClinicUpdated}:{profile:Profile;role:Role;clinic:Clinic;onUpdated:(p:Profile)=>void;onClinicUpdated:(c:Clinic)=>void}){
  ...
  useEffect(()=>{setProfileForm(f=>({...f,full_name:profile.full_name,email:profile.email||"",phone:profile.phone||"",preferred_locale:profile.preferred_locale}))},[profile]);
  useEffect(()=>{setClinicForm(f=>({...f,name:clinic.name,phone:clinic.phone||"",email:clinic.email||"",address:clinic.address||"",website:clinic.website||"",timezone:clinic.timezone||"Europe/Istanbul",default_locale:clinic.default_locale,booking_horizon_days:String(clinic.booking_horizon_days??60),minimum_booking_notice_hours:String(clinic.minimum_booking_notice_hours??2),cancellation_notice_hours:String(clinic.cancellation_notice_hours??12),allow_client_cancellation:clinic.allow_client_cancellation??true,allow_online_booking:clinic.allow_online_booking??true}))},[clinic]);
  useEffect(()=>{let active=true;(async()=>{const{data:p}=await supabase.from("profiles").select("notification_preferences").eq("id",profile.id).maybeSingle();if(!active)return;const prefs=(p?.notification_preferences||{}) as Record<string,boolean|string>;setProfileForm(f=>({...f,email_notifications:Boolean(prefs.email_notifications??true),appointment_reminders:Boolean(prefs.appointment_reminders??true),meal_reminders:Boolean(prefs.meal_reminders??true),weekly_summary:Boolean(prefs.weekly_summary??true),sms_notifications:Boolean(prefs.sms_notifications??false),appointment_changes:Boolean(prefs.appointment_changes??true),loyalty_notifications:Boolean(prefs.loyalty_notifications??true),progress_insights:Boolean(prefs.progress_insights??true),default_appointment_mode:String(prefs.default_appointment_mode||"in_clinic")}));if(role==="client"){const{data:c}=await supabase.from("client_profiles").select("id,birth_date,gender,height_cm,target_text,allergies,disliked_foods,medical_notes,medications,community_opt_in").eq("clinic_id",clinic.id).eq("user_id",profile.id).eq("is_active",true).maybeSingle();if(!active)return;if(c){setClientForm({birth_date:c.birth_date||"",gender:c.gender||"",height_cm:c.height_cm==null?"":String(c.height_cm),target_text:c.target_text||"",allergies:(c.allergies||[]).join(", "),disliked_foods:(c.disliked_foods||[]).join(", "),medical_notes:c.medical_notes||"",medications:c.medications||"",community_opt_in:c.community_opt_in??true});const{data:paymentRows,error:paymentError}=await supabase.from("payments").select("id,service_type,description,amount,paid_amount,remaining_amount,currency,status,method,paid_at,due_date,reminder_sent_at,created_at").eq("client_id",c.id).order("created_at",{ascending:false});if(!active)return;if(paymentError)setMessage(paymentError.message);else setClientPayments((paymentRows||[]) as ClientPaymentRow[])}}if(role==="owner"||role==="dietitian"){const{data:d}=await supabase.from("dietitian_profiles").select("title,license_no,bio,appointment_duration_minutes,buffer_minutes,is_bookable").eq("clinic_id",clinic.id).eq("user_id",profile.id).maybeSingle();if(active&&d)setDietForm({title:d.title||"Diyetisyen",license_no:d.license_no||"",bio:d.bio||"",appointment_duration_minutes:String(d.appointment_duration_minutes),buffer_minutes:String(d.buffer_minutes),is_bookable:d.is_bookable})}})();return()=>{active=false}},[clinic.id,profile.id,role,supabase]);
  async function saveProfile(){const fullName=profileForm.full_name.trim();if(fullName.length<2)return setMessage("Ad soyad en az 2 karakter olmalıdır.");const email=profileForm.email.trim();if(email&&!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))return setMessage("E-posta adresi geçersiz.");if(!locales.includes(profileForm.preferred_locale))return setMessage("Panel dili geçersiz.");const prefs={email_notifications:profileForm.email_notifications,appointment_reminders:profileForm.appointment_reminders,meal_reminders:profileForm.meal_reminders,weekly_summary:profileForm.weekly_summary,sms_notifications:profileForm.sms_notifications,appointment_changes:profileForm.appointment_changes,loyalty_notifications:profileForm.loyalty_notifications,progress_insights:profileForm.progress_insights,default_appointment_mode:profileForm.default_appointment_mode};const{data,error}=await supabase.from("profiles").update({full_name:fullName,email:email||null,phone:profileForm.phone.trim()||null,preferred_locale:profileForm.preferred_locale,notification_preferences:prefs}).eq("id",profile.id).select("id,full_name,email,phone,preferred_locale").single();if(error)setMessage(error.message);else{onUpdated(data as Profile);setMessage("Hesap ayarları kaydedildi.")}}
  async function saveClient(){const list=(v:string)=>v.split(",").map(x=>x.trim()).filter(Boolean).slice(0,100);const height=clientForm.height_cm?Number(clientForm.height_cm):null;if(height!==null&&(!Number.isFinite(height)||height<80||height>250))return setMessage("Boy 80 ile 250 cm arasında olmalıdır.");const{error}=await supabase.from("client_profiles").update({birth_date:clientForm.birth_date||null,gender:clientForm.gender||null,height_cm:height,target_text:clientForm.target_text.trim().slice(0,500)||null,allergies:list(clientForm.allergies),disliked_foods:list(clientForm.disliked_foods),medical_notes:clientForm.medical_notes.trim().slice(0,5000)||null,medications:clientForm.medications.trim().slice(0,5000)||null,community_opt_in:clientForm.community_opt_in,updated_at:new Date().toISOString()}).eq("clinic_id",clinic.id).eq("user_id",profile.id).eq("is_active",true);if(error)setMessage(error.message);else setMessage("Danışan profiliniz güncellendi.")}
  async function saveClinic(){const name=clinicForm.name.trim();if(name.length<2)return setMessage("Klinik adı en az 2 karakter olmalıdır.");const email=clinicForm.email.trim();if(email&&!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))return setMessage("E-posta adresi geçersiz.");const bookingHorizon=Number(clinicForm.booking_horizon_days),minimumNotice=Number(clinicForm.minimum_booking_notice_hours),cancellationNotice=Number(clinicForm.cancellation_notice_hours);if(!Number.isInteger(bookingHorizon)||bookingHorizon<1||bookingHorizon>365)return setMessage("Randevu ufku 1 ile 365 gün arasında olmalıdır.");if(!Number.isFinite(minimumNotice)||minimumNotice<0||minimumNotice>720)return setMessage("Minimum rezervasyon süresi 0 ile 720 saat arasında olmalıdır.");if(!Number.isFinite(cancellationNotice)||cancellationNotice<0||cancellationNotice>720)return setMessage("İptal süresi 0 ile 720 saat arasında olmalıdır.");const payload={name,phone:clinicForm.phone.trim()||null,email:email||null,address:clinicForm.address.trim().slice(0,2000)||null,website:clinicForm.website.trim().slice(0,500)||null,timezone:clinicForm.timezone.trim()||"Europe/Istanbul",default_locale:clinicForm.default_locale,booking_horizon_days:bookingHorizon,minimum_booking_notice_hours:minimumNotice,cancellation_notice_hours:cancellationNotice,allow_client_cancellation:clinicForm.allow_client_cancellation,allow_online_booking:clinicForm.allow_online_booking};const{data,error}=await supabase.from("clinics").update(payload).eq("id",clinic.id).select("id,name,slug,default_locale,timezone,phone,email,address,website,booking_horizon_days,minimum_booking_notice_hours,cancellation_notice_hours,allow_client_cancellation,allow_online_booking").single();if(error)setMessage(error.message);else{onClinicUpdated(data as Clinic);setMessage("Klinik ayarları kaydedildi.")}}
  async function saveDiet(){const duration=Number(dietForm.appointment_duration_minutes),buffer=Number(dietForm.buffer_minutes);if(!Number.isInteger(duration)||duration<10||duration>240)return setMessage("Randevu süresi 10 ile 240 dakika arasında olmalıdır.");if(!Number.isInteger(buffer)||buffer<0||buffer>120)return setMessage("Mola süresi 0 ile 120 dakika arasında olmalıdır.");const title=dietForm.title.trim();if(title.length<2)return setMessage("Unvan en az 2 karakter olmalıdır.");const{error}=await supabase.from("dietitian_profiles").update({title:title.slice(0,180),license_no:dietForm.license_no.trim().slice(0,180)||null,bio:dietForm.bio.trim().slice(0,5000)||null,appointment_duration_minutes:duration,buffer_minutes:buffer,is_bookable:dietForm.is_bookable}).eq("clinic_id",clinic.id).eq("user_id",profile.id);if(error)setMessage(error.message);else setMessage("Diyetisyen ayarları kaydedildi.")}
