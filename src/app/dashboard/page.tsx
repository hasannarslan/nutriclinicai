...
export default async function DashboardPage() {
  ...
  const [{ data: profile }, { data: membership }] = await Promise.all([
    ...
    supabase.from("clinic_memberships").select("id,clinic_id,user_id,role,is_active").eq("user_id", user.id).eq("is_active", true).single(),
  ]);
  ...
  if (!profile || !membership) {
    return (
      <main className="center-state">
        ...
        <p>Kayıt profili henüz oluşmadı. Sayfayı birkaç saniye sonra yenileyin. Sorun devam ederse Supabase migration dosyasını kontrol edin.</p>
      </main>
    );
  }
  ...
  return <DashboardClient initialProfile={profile as Profile} initialMembership={membership as Membership} clinic={clinic as Clinic} />;
}
